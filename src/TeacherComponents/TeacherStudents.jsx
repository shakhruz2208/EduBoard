...
import { BiSearch, BiChevronLeft, BiChevronRight, BiDotsVerticalRounded } from "react-icons/bi"
...
const TeacherStudents = () => {
  ...
  const studentsWithProgress = useMemo(() => students
    ...
    .map((student) => {
      ...
      return {
        ...student,
        score: graded.reduce((sum, submission) => sum + Number(submission.grade), 0),
    ...
    }), [assignmentsList, selectedCourse, students, submissions])
              ...
              {visibleStudents.map((student) => {
                ...
                return (
                  <div key={student.id} className="flex items-center gap-4 px-5 sm:px-7 py-4 hover:bg-indigo-900/20 transition-colors">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center font-bold shrink-0">{student.full_name?.[0]?.toUpperCase() || "S"}</div>
                    <div className="min-w-0 w-44 shrink-0"><p className="text-white font-semibold truncate">{student.full_name}</p><p className="text-xs text-indigo-300 truncate">{student.email}</p></div>
